...
def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_summed_amount
  "SELECT users.name, SUM(pledges.amount) AS total_pledged FROM users INNER JOIN pledges on users.id = pledges.user_id GROUP BY users.name ORDER BY SUM(pledges.amount)"
end
